...
- `methodology_versions.mix_v2` is the active scoring version (ADR 0003). Live prompt is `score_v3`. `mix_v1` remains as a retired historical row. `tag_stats` and `tag_trends` count only the six live tags. `claim_rescore_prompt` is `service_role` only.
- Hosted cron ticks `uap-ingest-news` (`*/15`), `uap-gate-stories`, `uap-translate-stories`, `uap-match-entities`, `uap-score-stories`, `uap-ingest-podcasts`, and `uap-ingest-x` via `uap_scheduler_tick`. The Vault name is `uap_scheduler_secret` and must match the Edge Function secret `UAP_SCHEDULER_SECRET`. Do not commit the value.
- `stories.relevance_status` (`pending | accepted | rejected`) and `gate_attempts` are service-role only; no anon grant and not on `stories_public`. `claim_pending_stories` requires accepted. `claim_gate_stories` locks only (does not change status).
- `stories.language`, `title_original`, `translation_status` (`pending | original | translated | failed`), and `translate_attempts` are service-role only; no anon grant and not on `stories_public`. Existing rows were backfilled `en` / `original`. `claim_pending_stories` requires `translation_status in (original, translated)`. `claim_translate_stories` locks only. `claim_rescore_stories` also excludes `translate_failed`.
