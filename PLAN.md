...
- Data ownership: `dataset/` is the offline source of truth. `supabase/migrations/` owns DDL/RLS/grants/views. `tools/seed-registry.mjs` owns hosted import (service role only). `src/app/globals.css` maps Graphite tokens. Components consume tokens only.
