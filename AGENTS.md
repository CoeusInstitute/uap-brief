...
- `supabase/functions/` owns Edge Function contracts (`ingest-news`, `gate-stories`, `translate-stories`, `match-entities`, `score-stories`, `ingest-podcasts`, `ingest-x`, `enrich-dossier`, `enrich-x`).
...
- Models: OpenRouter, `deepseek/deepseek-v4.1-flash`, `reasoning.effort` `high`. Do not send `temperature`. Scoring methodology is `mix_v2` / `score_v3` per [docs/adr/0003-tag-set-mix-v2.md](docs/adr/0003-tag-set-mix-v2.md). Only Edge Functions call OpenRouter or X.
- Design: Current look lives in the app CSS (`src/app/globals.css` and `frontend_design/graphite-ui.css`). There is no UI kit contract. Agents may evolve the interface from the existing dark, low-glare palette.
- Front door: `/` is a news feed. Composition follows The Bias Brief (sticky search, lead slot with featured image, 16:9 thumbnail story rows). `/` is single-column with no aside; the owner removed the Watching, Latest episodes, and Signal from X rails and the tag / form chip rows (2026-09-16). Do not copy TBB classes, Newsreader, or bias chips. No page display title on `/`; the masthead wordmark is enough. Primary nav label is Feed. The feed and `/story/[id]` read `stories_public`. The sticky bar is the search field. Each Ready row and the lead show an assessment line: one caution tag (`WOO`, `LACKING_DATA`, or `PSYOP`) paired with one substance tag (`CREDIBLE`, `INTERESTING`, or `VETTED`). Stored `WOO` displays as Unlikely (`tagLabel`); do not print Woo/WOO in the UI. Score chips stay on `/story/[id]` Scores and Method. Each row shows `summary` (the model brief, 2–5 sentences) and falls back to the cleaned excerpt only while a brief is missing. Images come from `image_url` when `image_status = stored`; otherwise a row keeps an empty image well and the lead omits the media block; never a stock placeholder. Lead prefers `form = news`, then excludes UFO Sightings Daily unless it is the only row. `ingest-x` still stores tracked-account posts for person pages. Empty corpus stays empty.
...
- Phase: P1 hosted schema and registry seed are applied. P4 reading desk is in place. P5 graph is the appearance network (2D, not 3D). P2/P3/P6 pipelines are unblocked (owner, 2026-09-16): news ingest, topic gate, English translate, entity match, hybrid scoring, podcast RSS/YouTube feeds, official X API. Do not invent RSS URLs, subscriber counts, follower counts, stories, or scores. Empty feed stays empty until `stories_public` has Ready rows.
